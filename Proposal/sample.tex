...
Three evidence branches are toggled on/off to give eight configurations
(E0--E7), matching the abstract's ``eight experimental configurations'':
\emph{general} (GMM, DTW), \emph{cue} (VOT, F0$_{\text{pert}}$, H1H2,
AspDur, Fuzzy), and \emph{context} (Topic). E0 is a majority-class
baseline; E7 is the full hybrid model.
...
\begin{enumerate}
  \item Lane 1 (Utsab) delivers real VOT, F0$_{\text{pert}}$, H1H2, and
        AspDur extraction per occurrence;
  \item Lane 2 (Sushank) delivers real GMM, DTW, Fuzzy, and Topic scores
        in the Candidate $\times$ Occurrence Evidence Table;
